...
class SurvivalFFNNLLL21(SurvivalNeuralNet):
    def __init__(self, model_name, cancer, omics_type, out_folder, epochs=1000, vecdim=10):
        super(SurvivalFFNNLLL21, self).__init__(model_name, cancer, omics_type, out_folder, epochs)
        self.vecdim = vecdim
    ...
    def feature_selection(self, x, c, s, xnames, fold, sel_f_num, dev_index):  
        def get_sel_idx(high_th_year, low_th_year, feature_list, sel_feature_num):
            ...
            W, _, _ = ll_l21.proximal_gradient_descent(trn_x, trn_y, z=0.01, mode='raw')
            sort_idx = feature_ranking(W)            

            return sort_idx[:sel_feature_num]

        save_feature_file = self.out_folder+'/FFNNLLL21/selected_features_'+self.cancer_type+'_'+self.omics_type+'_'+str(fold)+'.csv'
